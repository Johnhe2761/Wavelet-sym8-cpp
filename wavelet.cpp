...
template<typename T>
void Decomposition1D<T>::SetLen(const std::vector<T>& l)
{
    this->L = l;
}
template<typename T>
void Decomposition1D<T>::SetCoef(const std::vector<std::vector<T>>& c)
{
    this->C = c;
}
template<typename T>
const std::vector<T>& Decomposition1D<T>::GetLen()
{
    return this->L;
}
template<typename T>
const std::vector<std::vector<T>>& Decomposition1D<T>::GetCoef()
{
    return this->C;
}
//=============================================================================
// Wavelet constructor and destructor
//=============================================================================
...
Decomposition1D<T> Wavelet<T>::Wavedec(const std::vector<T>& x, size_t num_levels) const
{
    ...
    for( int k = num_levels-1; k >=0  ;-- k){
        coef.push_back(decomposition.GetDetcoef(k));
    }
    decomposition.SetLen(len);
    decomposition.SetCoef(coef);
...
template<typename T>
std::vector<T> Wavelet<T>::Wrcoef(const Decomposition1D<T>& decomposition,  size_t level) const
{
    return Appcoef(decomposition , level);
}
template<typename T>
std::vector<T> Wavelet<T>::Wrcoef(const Decomposition1D<T>& decomposition, std::vector<T> l,std::vector<std::vector<T>> c ,size_t level, COEFTYPE coeftype) const
{
    if(coeftype == a)
        return Wrcoef(decomposition,level);
    else if(coeftype == d){
        int iMin = l.size() - level;
        std::vector<T> coef = c.at(iMin-1);
        coef = Upconv(coef , Hi_R_ , l[iMin]);
        for(int k = 1; k < level ; ++k){
            coef = Upconv(coef , Lo_R_ , l[iMin + k] );
        }
        return coef;
    }
}
