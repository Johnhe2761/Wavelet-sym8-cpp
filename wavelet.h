...
public:
    ...
    void SetLen(const std::vector<T>& l);
    void SetCoef(const std::vector<std::vector<T>>& c);
    const std::vector<T>& GetLen();
    const std::vector<std::vector<T>>& GetCoef();
...
public:
    ...
    std::vector<T> Wrcoef(const Decomposition1D<T>& decomposition,  size_t level) const;
    std::vector<T> Wrcoef(const Decomposition1D<T>& decomposition,  std::vector<T> l,std::vector<std::vector<T>> c , size_t level,COEFTYPE coeftype) const;
